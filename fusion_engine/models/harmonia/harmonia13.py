...
class HarmoniaLiteEngine:
    ...
    def train(self, data=None, epochs: int = 1) -> Dict[str, Any]:
        """
        真正的"训练"：智能路由新语料到对应专家 + 自动关键词扩展。
        
        改进：
        1. 智能路由：每个片段根据关键词匹配分配给最合适的专家
        2. 关键词扩展：从新分配的片段中提取新关键词
        3. 质量控制：过滤低质量片段（太短、纯代码、无意义）
        """
        before = len(self._learned_fragments)
        new_frags = self._extract_fragments(data)
        
        # 质量过滤
        quality_frags = self._quality_filter(new_frags)
        if not quality_frags:
            return {"fragments_learned": 0, "total_learned": len(self._learned_fragments), "epochs": epochs}
        
        # 智能路由到专家
        routing_stats = self._route_to_experts(quality_frags)
        
        # 更新 learned_fragments
        self._learned_fragments.extend(quality_frags)
        
        # 为每个有新片段的专家提取新关键词
        keywords_added = 0
        for exp_id, frags in routing_stats["routed"].items():
            if frags and exp_id != "general":
                exp = self._find(exp_id)
                if exp:
                    new_kws = self._extract_keywords(frags, exp["keywords"])
                    if new_kws:
                        exp["keywords"].extend(new_kws)
                        keywords_added += len(new_kws)
            elif exp_id == "general" and frags:
                general = self._find("general")
                if general:
                    # general 也提取新关键词
                    new_kws = self._extract_keywords(frags, general["keywords"])
                    if new_kws:
                        general["keywords"].extend(new_kws)
                        keywords_added += len(new_kws)
        
        # 为 general 专家补充新领域关键词（如果 general 有新片段但关键词不足）
        general = self._find("general")
        if general and routing_stats.get("unassigned", 0) > 0:
            unassigned_frags = routing_stats.get("unassigned_frags", [])
            if unassigned_frags:
                new_kws = self._extract_keywords(unassigned_frags, general["keywords"])
                if new_kws:
                    general["keywords"].extend(new_kws)
                    keywords_added += len(new_kws)
        ...
        return {
            ...
            "epochs": epochs,
            "routing": routing_stats.get("stats", {}),
            "keywords_added": keywords_added,
        }
    
    def _quality_filter(self, fragments: List[str]) -> List[str]:
        """质量过滤：去除低质量片段"""
        filtered = []
        for frag in fragments:
            ...
            if len(frag) > 600:
                frag = frag[:600]
            # 纯代码检测（过多符号）
            code_chars = sum(1 for c in frag if c in '#{}[]()<>\\|/=+*&^%$@!~`')
            if code_chars > len(frag) * 0.3:
                # 纯代码片段，但如果包含注释或文档说明，保留
                if not any(w in frag.lower() for w in ['#', '//', '"""', "'''", '"""', 'note', 'example', 'usage', '说明', '注释', '用法', '示例']):
                    continue
            # 无意义检测（太多连续相同字符或太短）
            if len(set(frag)) < 10:
                continue
    ...
    def _route_to_experts(self, fragments: List[str]) -> Dict[str, Any]:
        """智能路由：根据关键词匹配将片段分配给最合适的专家"""
        ...
        for frag in fragments:
            ...
            if best_score > 0 and best_exp["id"] != "general":
                # 有明确的专家匹配
                routed.setdefault(best_exp["id"], []).append(frag)
                best_exp["fragments"].append(frag)
            else:
                # 无明确匹配，路由到 general
                general = self._find("general")
                if general:
                    general["fragments"].append(frag)
                    routed.setdefault("general", []).append(frag)
                unassigned.append(frag)
                unassigned_frags.append(frag)
        
        # 统计
        stats = {}
        for exp_id, frags in routed.items():
            stats[exp_id] = len(frags)
        
        return {
            "routed": routed,
            "stats": stats,
            "unassigned": len(unassigned),
            "unassigned_frags": unassigned_frags,
        }
    
    def _extract_keywords(self, fragments: List[str], existing_keywords: List[str]) -> List[str]:
        """从片段中提取新关键词"""
        import re
        
        # 统计现有关键词
        existing = set(k.lower() for k in existing_keywords)
        ...
        for frag in fragments:
            ...
            # 提取技术术语模式（如 my_variable, some_func）
            tech_words = re.findall(r'[a-zA-Z][a-zA-Z0-9_]+', frag.lower())
            
            for w in words + tech_words:
                # 跳过太短或常见词
                if len(w) < 3 or w in existing:
                    continue
                if w in {'the', 'and', 'for', 'with', 'this', 'that', 'from', 'have', 'will', 'your', 'been', 'were', 'are', 'was', 'not', 'can', 'use', 'used', 'using', 'used', 'some', 'what', 'when', 'where', 'how', 'why', 'does', 'did', 'just', 'new', 'also', 'get', 'make', 'like', 'more', 'most', 'other', 'only', 'over', 'such', 'take', 'than', 'them', 'then', 'here', 'there', 'about', 'into', 'through', 'because'}:
                    continue
                word_counts[w] = word_counts.get(w, 0) + 1
        
        # 选择高频词作为新关键词（至少出现2次）
        new_keywords = [w for w, c in sorted(word_counts.items(), key=lambda x: x[1], reverse=True) if c >= 2 and w not in existing]
        
        # 限制每个批次最多添加10个关键词，避免泛滥
        return new_keywords[:10]
    ...
    def _gate(self, prompt: str, terms: List[str], top_k: int) -> List[Dict[str, Any]]:
        ...
        策略：
        1. 专业专家（非 general）关键词匹配 +5 分（加权更高）
        2. general 专家只给 +2 分，作为兜底
        3. 优先选择有明确关键词命中的专业专家
        """
        p = prompt.lower()
        scored: List[tuple] = []
        for exp in self.experts:
            score = 0.0
            is_general = exp["id"] == "general"
            
            # 关键词匹配
            kws = exp["keywords"][:500] if len(exp["keywords"]) > 500 else exp["keywords"]
            kw_hits = 0
            for kw in kws:
                if kw.lower() in p:
                    kw_hits += 1
            ...
            if kw_hits > 0:
                if is_general:
                    score += kw_hits * 2.0  # general 权重较低
                else:
                    score += kw_hits * 5.0  # 专业专家权重更高
            
            # 片段术语重叠
            if terms and kw_hits > 0:  # 只有关键词命中时才扫描片段
                match = 0
                scan_limit = min(500, len(exp["fragments"]))
    ...
    def _retrieve(
        ...
        for exp in chosen:
            ...
            # general 片段多，限制扫描数量（采样）
            if exp["id"] == "general" and len(exp["fragments"]) > 5000:
                # 随机采样 5000 个片段，保证性能
                import numpy as np
                rng = np.random.default_rng(42)
                indices = rng.choice(len(exp["fragments"]), size=5000, replace=False)
                indices.sort()
                scan_frags = [exp["fragments"][i] for i in indices]
            else:
                scan_frags = exp["fragments"]
